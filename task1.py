...
def main():
    ...
    case_num = int(sys.argv[1]) #1
    support = int(sys.argv[2]) #4
    input_file_name = sys.argv[3] #"small2.csv"
    output_file_name = sys.argv[4] #"task1.txt"
    textRDD = sc.textFile(input_file_name)
    # Split Data
    textRDD = textRDD.mapPartitionsWithIndex(lambda index, data: itertools.islice(data, 1, None) if index == 0 else data)
    # Provide (SON) each mapper with a single key­value pair
    ...
    ## Second Phase of SON: Candidate Filtering (class: list)
      # main goal: Remove the candidates from the n  n first phase that are false positives
    freq_Itemsets = textRDD.mapPartitions(lambda basket: Count_num_occurrence(basket, candidate_freq_itemsets)).reduceByKey(
